...
# =============================================================================================
PercnetageList = sorted([a for a in range(10, 101, 10)] + [37])
Testtimes = 30
    ...
    for time1000 in range(Testtimes):
        ...
        while True:
            ...
            SDDataFrame = DataFrame({
                "FilteredNumber": FilterDataFrame["FilteredNumber"],
            })  # statistic the standard deviation
            SDList = []

            shuffle(FilterDataFrame["FilteredNumber"])
            for CandidateNumber37 in FilterDataFrame["FilteredNumber"][:(5040 * percnetage) // 100]:
            ...
            SDDictionary = dict(
                zip(FilterDataFrame["FilteredNumber"][:(5040 * percnetage) // 100], SDList))

            minSD37 = min(SDList)
            IndexNumber37 = list(SDDictionary.values()).index(minSD37)
            GuessNumber = list(SDDictionary.keys())[IndexNumber37]

            for CandidateNumber63 in FilterDataFrame["FilteredNumber"][(5040 * percnetage) // 100:]:
