...
for percnetage in PercnetageList:
    ...
    for time1000 in range(Testtimes):
        ...
        while True:
            ...
            maxSD37 = max(SDList)
            IndexNumber37 = list(SDDictionary.values()).index(maxSD37)
    ...
    with open("20220204record.txt", 'a') as f:
    ...
    plt.savefig(f"20220204{ percnetage }%Frequency.png")
    plt.close()
    ...
    plt.bar(XTime, YTime)
    plt.savefig(f"20220204{ percnetage }%Time.png")
    plt.close()
